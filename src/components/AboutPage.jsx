...
const AboutPage = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#1a120b] via-[#3c2a21] to-[#1a120b] text-amber-50 overflow-hidden relative">
      {/* Removed mix-blend-soft-light from here */}
      <motion.section
      ...
      >
        <div className="max-w-4xl mx-auto">
          <motion.h1 className="text-5xl sm:text-6xl md:text-7xl font-bold mb-4 font-serif bg-clip-text text-transparent capitalize bg-gradient-to-r from-yellow-400 via-amber-300 to-yellow-200 drop-shadow-lg">
            culinary express
          </motion.h1>
          <motion.p
            initial={{ opacity: 0 }}
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 md:gap-12">
          {features.map((feature, index) => {
            ...
            return (
              ...
              >
                <div className="absolute inset-0 bg-gradient-to-r from-amber-600/30 to-amber-500/30 rounded-3xl blur-lg opacity-50 group-hover:opacity-80 transition-opacity duration-500" />
                <div className="bg-[#3c2a21]/90 backdrop-blur-lg rounded-3xl overflow-hidden border border-amber-600/30 hover:border-amber-500 transition-all duration-300 h-full">
          ...
          {stats.map((stat, index) => {
            ...
            return (
              <motion.div
                ...
                whileHover={{ scale: 1.05 }}
                onHoverStart={() => setHoverStats(index)}
                onHoverEnd={() => setHoverStats(null)}
                className="relative z-0"
              >
                <motion.div
                  className="relative h-full bg-[#3c2a21]/40 backdrop-blur-lg rounded-xl border-2 border-amber-600/30 p-6 overflow-hidden transition-all duration-300 shadow-lg"
                  animate={{
                    zIndex: hoverStats === index ? 10 : 1,
                  }}
                >
                  {/* Animated Background */}
                  <motion.div
                  ...
                  <div className="relative z-10 h-full flex flex-col items-center justify-center">
                    ...
                    <motion.div
                      ...
                      animate={{
                        letterSpacing:
                          hoverStats === index ? "0.15rem" : "0.1rem",
                        textShadow:
                          hoverStats === index
                  ...
                  <motion.div
                    ...
                    initial={{ opacity: 0 }}
                    animate={{ opacity: hoverStats === index ? 1 : 0 }}
                  />
                </motion.div>
                <motion.div
                  className=" absolute inset-0 bottom-0 h-8 bg-amber-900/30 blur-xl rounded-full"
                  animate={{
                    opacity: hoverStats === index ? 0.4 : 0.2,
                    scale: hoverStats === index ? 0.9 : 0.8,
                  }}
                />
              </motion.div>
            );
      ...
      <section className=" py-16 px-4 md:px-8 relative">
        <div className=" max-w-7xl  mx-auto ">
          ...
          >
            Meet Me <span className=" text-amber-500">Culinary Artists</span>
          </motion.h2>
          <div className=" grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 lg:gap-12">
            {teamMembers.map((member, index) => (
              <motion.div
                ...
                initial={{ opacity: 0, y: 50 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true, margin: "0px -100px 0px" }}
