...
function Banner() {
  ...
  return (
    <div className="relative">
      <div className="bg-gradient-to-br from-amber-900 via-amber-800 to-amber-700 text-white px-4 py-16 md:px-8 relative overflow-hidden">
        ...
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center gap-12 relative z-10 ">
          ...
          <div className="flex-1 space-y-8 relative md:pr-8 lg:pr-19 text-center md:text-left">
            <h1 className="text-4xl sm:text-5xl md:text-4xl lg:text-[52px] font-bold leading-tight font-serif drop-shadow-md ">
              {"We're Here "} <br />
            ...
            <p className="text-lg md:text-lg lg:text-xl font-playfair italic sm:text-xl text-amber-100 max-w-xl opacity-90 mx-auto md:mx-0">
              Best cooks and best deliveries all at your service. Hot tasty food
            ...
            >
              <div className="relative flex items-center bg-amber-900/30 rounded-xl border-2 border-amber-500 shadow-2xl hover:bg-amber-400/50 transition-all duration-300">
                ...
                <input
                  ...
                  placeholder="Discover Your Favourite Meal..."
            ...
            <div className="flex flex-wrap gap-4 justify-center md:justify-start mt-6">
              <button className="group flex items-center backdrop-blur-sm gap-3  bg-amber-800/30 hover:bg-amber-800/50 px-6 py-3 rounded-xl transition-all duration-300 border-2 border-amber-700/50 hover:border-amber-400">
                <IoCloudDownloadOutline className="text-xl text-amber-400 group-hover:animate-bounce" />
          ...
          {/* RIGHT IMAGE CONTENT WITH ORBITAL IMAGES */}
          <div className="flex-1 relative group mt-8 md:mt-0 min-h-[300px] sm:min-h-[400px] ">
            {/* MAIN IMAGE */}
            <div className="relative rounded-full p-1 bg-gradient-to-br from-amber-700 via-amber-800 to-amber-900 shadow-2xl z-20 w-[250px] sm:w-[350px] xs:w-[300px] h-[250px] sm:h-[350px] xs:h-[300px] mx-auto ">
              <img
                src={bannerImage}
                alt="banner"
                className="border-4 rounded-full xs:border-8  border-amber-900/50 w-full h-full object-cover object-top"
              />
              <div className="aabsolute inset-0 rounded-full bg-gradient-to-b from-transparent to-amber-900/40 mix-blend-multiply"></div>
            </div>
            {/* ORBITAL IMAGES */}
            {orbitImages.map((images, index) => {
              return (
                <div
                  key={index}
                  className={`absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 ${
                    index === 0 ? "orbit" : `orbit-delay-${index * 5}`
                  } w-[80px] sm:w-[150px] xs:w-[100px] h-[80px] xs:h-[100px] sm:h-[150px]`}
                >
                  <img
                    src={images}
                    className="w-full h-full border-amber-500/30 shadow-lg bg-amber-900/20 p-1 object-cover"
